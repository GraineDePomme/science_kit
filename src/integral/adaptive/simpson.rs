...
pub fn simpson<F>(f: F, a: f64, b: f64, abs_error: f64, rel_error: f64) -> Measure
where
    F: Fn(f64) -> f64,
{
    ...
    let mut temp: Measure = crate::integral::non_adaptive::simpson(&f, a, (b + a) / 2.0, 10);
    result += temp.value;
    error += temp.error;
    ...
    temp = crate::integral::non_adaptive::simpson(&f, (b + a) / 2.0, b, 10);
    result += temp.value;
    error += temp.error;
    intervals.push(Interval {
        a: (b + a) / 2.0,
    ...
    while error >= abs_error && error >= rel_error * result.abs() {
        ...
        temp = crate::integral::non_adaptive::simpson(&f, lower_bound, middle, 10);
        intervals.push(Interval {
            a: lower_bound,
            b: middle,
            value: temp.value,
        ...
        temp = crate::integral::non_adaptive::simpson(&f, middle, upper_bound, 10);
        intervals.push(Interval {
            a: middle,
            b: upper_bound,
            value: temp.value,
